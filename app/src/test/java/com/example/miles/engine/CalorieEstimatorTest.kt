...
class CalorieEstimatorTest {
    ...
    fun `weight is clamped to a safe range`() {
        assertEquals(30.0, CalorieEstimator.clampWeight(5.0), 0.001)
        assertEquals(70.0, CalorieEstimator.clampWeight(70.0), 0.001)
        assertEquals(250.0, CalorieEstimator.clampWeight(9999.0), 0.001)
    }

    @Test
    fun `long hard session burns more than a short one`() {
        val short = CalorieEstimator.activityCalories(
            met = 8.0, weightKg = 70.0, elapsedSeconds = 600,
            distanceMeters = 1000.0, steps = 1200, perMetreRate = 0.065
        )
        val long = CalorieEstimator.activityCalories(
            met = 8.0, weightKg = 70.0, elapsedSeconds = 3600,
    ...
    fun `a real workout always reports at least one kcal after ten seconds`() {
        val cals = CalorieEstimator.activityCalories(
            met = 3.5, weightKg = 70.0, elapsedSeconds = 11,
    ...
    fun `a fresh start reports zero rather than fake calories`() {
        val cals = CalorieEstimator.activityCalories(
            met = 3.5, weightKg = 70.0, elapsedSeconds = 0,
            distanceMeters = 0.0, steps = 0, perMetreRate = 0.045
        )
        assertEquals(0, cals)
    }
    ...
    fun `ten thousand everyday steps gives a realistic burn for 70kg`() {
        val kcal = CalorieEstimator.dailyActiveFromSteps(10_000, 70.0)
    ...
    fun `everyday calories scale with steps and body weight`() {
        val tenK = CalorieEstimator.dailyActiveFromSteps(10_000, 70.0)
        val fiveK = CalorieEstimator.dailyActiveFromSteps(5_000, 70.0)
        val heavy = CalorieEstimator.dailyActiveFromSteps(10_000, 100.0)
    ...
    fun `no everyday steps means no calories`() {
        assertEquals(0, CalorieEstimator.dailyActiveFromSteps(0, 70.0))
        assertEquals(0, CalorieEstimator.dailyActiveFromSteps(-10, 70.0))
    }
}